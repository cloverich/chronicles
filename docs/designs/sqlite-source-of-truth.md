...
4. **Indexer → derive + repair.** Replace the filesystem-walking indexer with a `derive(document)` step called from every write and import, plus a `rebuildDerived()` maintenance command that regenerates every derived table from `documents`. Remove startup walking, `needsFullReindex`, orphan/duplicate reconciliation, journal-directory scanning, and the UI-visible "reindex" affordance (keep a "repair" under maintenance).
5. **Importer: Chronicles source.** Teach the importer to ingest a Chronicles notes tree (journal directories of `<id>.md` with frontmatter): preserve IDs, `createdAt`/`updatedAt`, journal from directory, tags, note links, and attachments. Re-import of an existing ID is explicit (skip or replace), never inferred from file position. Importer terminates in the same write/derive transaction as step 3. Produce a report: created, skipped, replaced, errored, attachments copied. Notion and generic Markdown import keep working.
...
7. **Backup.** `backup(destDir)` uses `VACUUM INTO` (WAL-safe) and copies the attachments directory. Surfaced in preferences alongside export.
