...
Phases 1-3 are complete (issues #444-#450). See [docs/features/theming.md](../features/theming.md) for how the system works today, and [docs/qa/theming.md](../qa/theming.md) for verification checklists.

### Deferred items

- **FOUC mitigation** — Inject critical theme variables (background, foreground) from main process into `index.html` before React mounts. Brief dark flash confirmed on startup.
- **Per-journal contextual themes** — Co-locate `journal.theme.json` in journal directories; override global theme when journal is active. Requires path resolution, file watching, precedence rules.
- **Theme visualizer / preview** — Swatch preview in preferences UI showing the active palette.
- **Cross-system theme conversion / generation** — Documented spec + prompt template for LLMs to convert themes from VS Code, iTerm, etc. The `validate()` function is the key enabler.
- **CLI theme validation** — `yarn validate-theme <path>` for validating theme files outside the app. See QA doc gaps section.
