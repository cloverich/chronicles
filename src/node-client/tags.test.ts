...
type NodeClient = Awaited<ReturnType<typeof createClient>>;
...
async function seedTagDocs(c: NodeClient) {
...
describe("TagsClient", () => {
  ...
  test("all() returns empty array on fresh DB", async () => {
    const ctx = await makeClient();
    try {
      assert.deepStrictEqual(await ctx.client.tags.all(), []);
    } finally {
      ctx.cleanup();
    }
  });
  ...
  test("all() returns distinct sorted tags", async () => {
    ...
    assert.deepStrictEqual(await client.tags.all(), [
      "hobby",
      "personal",
      "work",
    ]);
  ...
  test("allWithCounts() returns empty array on fresh DB", async () => {
    const ctx = await makeClient();
    try {
      assert.deepStrictEqual(await ctx.client.tags.allWithCounts(), []);
    } finally {
      ctx.cleanup();
    }
  });
