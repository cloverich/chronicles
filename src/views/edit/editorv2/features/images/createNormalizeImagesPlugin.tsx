...
import { ELEMENT_IMAGE, ELEMENT_VIDEO } from "../../../../markdown/plate-types";

/**
 * Ensure a dropped image is always at the top level of the document.
 */
export const createNormalizeImagesPlugin = createPlatePlugin({
  key: "top-level-image-normalizer",
}).overrideEditor(({ editor }) => {
  const originalNormalizeNode = editor.normalizeNode as (
    entry: NodeEntry,
  ) => void;
  ...
  editor.normalizeNode = (entry: NodeEntry) => {
    ...
    if (
      ...
      [ELEMENT_IMAGE, ELEMENT_VIDEO].includes((node as any).type as string) &&
      path.length > 1
    ) {
      if ((node as any).type === ELEMENT_VIDEO) {
